var Layout = (function () {
    ...
    Layout.prototype.ressizeDependency = function (c) {
        ...
        $("[data-samewidth]").each(function (indx, element) {
            var targetSelector = $(element).attr("data-samewidth");
            $(element).css('width', $(targetSelector).prop('scrollWidth') + 'px');
            if ($(element).hasClass('scroll-div')) {
                if ($(targetSelector).prop('scrollWidth') <= $(targetSelector).width()) {
                    $(element).parent().hide();
                }
                else {
                    $(element).parent().show();
