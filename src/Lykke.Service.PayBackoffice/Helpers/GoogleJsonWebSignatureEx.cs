...
{
    // Copied from https://github.com/google/google-api-dotnet-client/blob/364ef9d891996d36f49db3764486806a8981a41a/Src/Support/Google.Apis.Auth/GoogleJsonWebSignature.cs
    // since in original version fields "hd", "email" and "email_verified" of jwt payload are absent.
    public class GoogleJsonWebSignatureEx
    {
        ...
        internal static async Task<Payload> ValidateInternalAsync(string jwt, IClock clock, bool forceGoogleCertRefresh, string certsJson)
        {
            ...
            // Rounding to upper integer and adds two extra second to allow google's clock and our clock be a little bit different
            var nowSeconds = (long)Math.Ceiling((clock.UtcNow - UnixEpoch).TotalSeconds);
            if (nowSeconds + 2 < payload.IssuedAtTimeSeconds.Value)
            {
                throw new InvalidJwtException("JWT is not yet valid.");
            }
            if (nowSeconds - 2 > payload.ExpirationTimeSeconds.Value)
            {
