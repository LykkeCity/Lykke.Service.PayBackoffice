...
namespace BackOffice
{
	...
	public class Bundler
	{
		...
		public static HtmlString Unpack(string bundlePath)
		{
			var configFile = Path.Combine("bundleconfig.json");
		...
		private static Bundle GetBundle(string fullPath, string bundlePath)
		{
			var file = new FileInfo(fullPath);
			if (!file.Exists)
				return null;

			var bundles = JsonConvert.DeserializeObject<IEnumerable<Bundle>>(File.ReadAllText(fullPath));
