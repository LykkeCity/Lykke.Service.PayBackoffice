...
namespace AzureRepositories.AuditLog
{
    ...
    public class AuditLogRepository : IAuditLogRepository
    {
        ...
        {
            if (from > to)
                return new List<ClientAuditLogData>(0);

            var filterFrom = new DateTime(from.Year, from.Month, from.Day);
            var filterTo = new DateTime(to.Year, to.Month, to.Day).AddDays(1).AddTicks(-1);

            var kycStatusChanges = await _tableStorage.GetDataAsync(
                        item => item.RecordType == recordType && item.CreatedTime >= filterFrom && item.CreatedTime <= filterTo);

            var clientLogEntities = kycStatusChanges.Select(logItem => new ClientAuditLogData(logItem, logItem.PartitionKey.Split('_')[1]));
